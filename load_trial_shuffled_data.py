...
def mutate(resp):
    ...
    for n in np.arange(np.shape(labels)[1]):
        shuffled_n = np.copy(labels[:,n,:])
        np.random.shuffle(shuffled_n)
        shuffled_labels[:,n,:] = shuffled_n
    return (np.concatenate(np.array(sequences)),np.concatenate(shuffled_labels, axis=1))
seq, lab = mutate(resp)
