...
class RecurrentActivityModel:
    ...
    def __init__(self, x, y, x_mean, is_testing=False, FLAGS):
        def squared(tensor):
        ...
        with tf.variable_scope('fwd_rnn'):
            fwd_cell = tf.nn.rnn_cell.LSTMCell(
                    ...
                    use_peepholes=True,
                    state_is_tuple=True)
            fwd_cell = tf.nn.rnn_cell.MultiRNNCell([fwd_cell] * self.num_layers,state_is_tuple=True)

            fwd_inputs = tf.zeros_like(self.y)
            fwd_out, fwd_final_state = tf.nn.dynamic_rnn(
                fwd_cell,
                fwd_inputs,
        ...
        out_mod = tf.reshape(fwd_out, [-1, self.n_use])
